...
class CaseCategoryServices {
  static async getCaseCategories() {
    return new Promise((resolve, reject) => {
      $.ajax({
        url: `${endpointURL}casecategories/get`,
        method: "GET",
        dataType: "json",
  ...
  static async getSelectedRecord(caseCategoryId) {
    return new Promise((resolve, reject) => {
      $.ajax({
        url: endpointURL + "casecategories/get/" + caseCategoryId, // Use the provided clientId parameter
        type: "GET",
        // data: { id: caseCategoryId }, // Use the provided clientId parameter
        success: (response) => {
          resolve(response.data);
        },
  ...
  static async populateClientCategories(selectElement) {
    if (!countryId) return;
    try {
      const response = await fetch(`${endpointURL}casecategories/get`, {
        method: "GET",
      });
      if (!response.ok) {
        throw new Error(
  ...
  static async deleteRecord(caseCategoryId) {
    try {
      ...
      if (confirmation) {
        const response = await fetch(
          `${endpointURL}casecategories/delete/${caseCategoryId}`,
          {
            method: "DELETE",
            headers: {
...
export default CaseCategoryServices;
